...
class VOApp(App):
    def build(self):
        ...
        self.R_wc = np.eye(3)     # global rotation (camera-to-world)
        self.t_wc = np.zeros((3, 1))  # global translation (camera position in world)
        self.scale = 1.0          # arbitrary scale factor, can be tuned
        ...
        try:
            ...
            if self.prev_gray is None or self.prev_R_imu is None:
                ...
                if R is not None and t is not None:
                    # Normalize translation (since monocular VO has no absolute scale)
                    t = t / (np.linalg.norm(t) + 1e-9)
                
                    # Accumulate global pose (camera-to-world)
                    self.R_wc = self.R_wc @ R
                    self.t_wc = self.t_wc + self.R_wc @ (t.reshape(3, 1) * self.scale)
                
                    # Extract position (x, y, z)
                    pos = self.t_wc.flatten()
                
                    # Update labels
                    self.R_label.text = f"R (relative):\n{np.array2string(R, precision=2)}"
                    self.t_label.text = f"Pose (world):\nX={pos[0]:.3f}, Y={pos[1]:.3f}, Z={pos[2]:.3f}"
                else:
                    self.R_label.text = "R: Estimation failed"
                    self.t_label.text = "Pose: Estimation failed"
